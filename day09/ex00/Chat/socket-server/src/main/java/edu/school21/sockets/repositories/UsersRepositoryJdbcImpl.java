...
//    @Override
//    public Optional<User> findByName(String name) {
////        User user = jdbcTemplate.query(String.format("SELECT * FROM users WHERE name = '%s';", name), new UserMapper()).stream().findFirst().orElse(null);
//        User user = jdbcTemplate.query(String.format("SELECT * FROM users WHERE name IN('%s');", name), new UserMapper()).stream().findFirst().orElse(null);
//        return (Optional.ofNullable(user));
//        //return Optional.of(jdbcTemplate.queryForObject("SELECT * FROM users WHERE name = ?", new UserMapper(), name));
//    }
//}
//
//class UserMapper implements RowMapper<User> {
